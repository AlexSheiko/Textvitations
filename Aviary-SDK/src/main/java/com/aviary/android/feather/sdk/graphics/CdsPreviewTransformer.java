...
public class CdsPreviewTransformer implements Transformation, Callable<Bitmap> {
    ...
    public CdsPreviewTransformer(
        String path, String displayName, String type, int inputDensity, int targetDensity, int maxW, int maxH) {
    ...
    public Bitmap transform(Bitmap bitmap) {
        ...
        if (AviaryCds.PACKTYPE_EFFECT.equals(mType)) {
            ...
            if (null != bitmap && bitmap.isMutable()) {
                ...
                canvas.drawText(mDisplayName,
                                (bitmap.getWidth() - bounds.width()) / 2,
                                bitmap.getHeight() - paint.descent() - 2,
                                paint);
            }
        }
