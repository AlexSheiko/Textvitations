...
final class BackgroundDrawThread extends Thread {
    ...
    static Logger logger = LoggerFactory.getLogger("BackgroundDrawThread", LoggerType.ConsoleLoggerType);
    ...
    public synchronized void setSingleTapAllowed(boolean value) {
    ...
    public synchronized void start(Bitmap src, Bitmap preview) {
        if (started) {
            return;
        }
        logger.info("start");
    ...
    public synchronized void singleTap(float radius, float[] points, BrushMode brushType) {
        if (!running) {
            return;
        }
    }

    public synchronized void pathStart(float radius, float[] points, BrushMode brushType) {
        if (!running) {
            return;
        }
                ...
                if (null != src) {
                    if (!src.isRecycled() && (dst != null ? !dst.isRecycled() : true)) {
                        ...
                        if (null != handler && !isInterrupted()) {
                            Moa.notifyPixelsChanged(current);
        ...
        while (!isInterrupted()) {
            ...
            if (mQueue.size() > 0 && !isInterrupted()) {
                ...
                if (!s) {
                    s = true;
                    if (null != handler) {
                        handler.sendEmptyMessage(AbstractPanel.PROGRESS_START);
                    }
                }
                firstPoint = true;
                ...
                while (element.size() > 0 || !element.isCompleted()) {
                    ...
                    if (firstPoint) {
                        firstPoint = false;
                        drawRect.set((int) (points[0] - radius * brushMultiplier),
                        ...
                        drawRect.set(Math.min(left1, left2), Math.min(top1, top2), Math.max(left1, left2), Math.max(top1, top2));
                        drawRect.sort();
                        ...
                        if (null != filter && !isInterrupted()) {
                            ...
                            float[] invalidationPoints = filter.draw(radius,
                                                                     0,
                                                                     pathIndex,
                            ...
                            if (null != invalidationPoints && invalidationPoints.length == 4) {
                                drawRect.set((int) invalidationPoints[0],
                ...
                if (s) {
                    if (null != handler && !isInterrupted()) {
                        handler.sendEmptyMessage(AbstractPanel.PROGRESS_END);
                    }
                    s = false;
                }
            }
