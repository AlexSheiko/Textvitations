...
public class ImageViewDrawableOverlay extends ImageViewTouch implements DropTarget {
    ...
    public static final float BLUR_MASK_RADIUS         = 4.0f;
    ...
    protected void onZoomAnimationCompleted(float scale) {
        Log.i(TAG, "onZoomAnimationCompleted: " + scale);
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (mOverlayView != null && (mOverlayView.getMode() != DrawableHighlightView.NONE
            && mOverlayView.getMode() != DrawableHighlightView.DELETE)) {
    ...
    public boolean onDown(MotionEvent e) {
        ...
        if (realNewSelection != null && mScaleWithContent) {
            ...
            if (!invalidSize) {
                ...
                float minSize = Math.min(minW, minH) * MIN_SIZE_INCREASE_RATIO;
                ...
                zoomTo(getScale() * diff,
                       displayRect.centerX(),
                       displayRect.centerY(),
                       (long) (DEFAULT_ANIMATION_DURATION * ANIMATION_INCREASE_RATIO));
                return true;
            }
        }
        ...
        if (mOverlayView != null) {
            ...
            if (edge != DrawableHighlightView.NONE) {
                if (edge != DrawableHighlightView.DELETE) {

                    if (edge == DrawableHighlightView.OPACITY) {
                        mOverlayView.setMode(DrawableHighlightView.OPACITY);
                    } else {
                    ...
                    mOverlayView.setMode(DrawableHighlightView.DELETE);
                }
                postInvalidate();
                if (mDrawableListener != null) {
    ...
    public void setImageDrawable(
        android.graphics.drawable.Drawable drawable, Matrix initialMatrix, float minZoom, float maxZoom) {
    ...
    public boolean removeHightlightView(DrawableHighlightView view) {
        Log.i(TAG, "removeHightlightView");
        for (int i = 0; i < mOverlayViews.size(); i++) {
    ...
    public void onDragEnter(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo) {
        ...
        mDropPaint.setMaskFilter(new BlurMaskFilter(BLUR_MASK_RADIUS, Blur.NORMAL));
