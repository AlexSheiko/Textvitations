...
public class MemePanel extends AbstractContentPanel
    implements OnEditorActionListener, OnClickListener, OnDrawableEventListener, OnLayoutChangeListener {
    ...
    private void onAddTopText() {
        ...
        int textSize = (int) (Math.min(rect.width(), rect.height()) / 7);
    ...
    private void onAddBottomText() {
        ...
        final float[] pts =
            new float[]{rect.left, rect.bottom - cropHeight - paddingBottom, rect.left + rect.width(), rect.bottom - paddingBottom};
        MatrixUtils.mapPoints(matrix, pts);
