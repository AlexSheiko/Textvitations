...
public class TiltShiftPanel extends AbstractContentPanel
    implements OnTiltShiftDrawListener, OnDrawableChangeListener, OnCheckedChangeListener {
    ...
    public void onCheckedChanged(AviaryHighlightImageButton button, boolean isChecked, boolean fromUser) {
        ...
        getContext().getTracker()
            .tagEventAttributes(ToolLoaderFactory.Tools.FOCUS.name().toLowerCase(Locale.US) + ": style_selected", attrs);
    }
    ...
    public void onDrawStart(
        float[] points, TiltShiftDrawMode mode, float radius, float angle, float left, float top, float right, float bottom) {
    ...
    static class DrawQueue extends LinkedBlockingQueue<float[]> {
        ...
        private          TiltShiftMode tiltShiftMode;
        private volatile boolean       completed;

        public DrawQueue(TiltShiftMode tiltShiftMode) {
            this.tiltShiftMode = tiltShiftMode;
            completed = false;
        }
    ...
    class BackgroundDrawThread extends Thread {
        ...
        public synchronized void drawStart(
            float[] points, TiltShiftDrawMode type, float radius, float angle, float left, float top, float right, float bottom) {
            if (!running) {
                return;
            }
        ...
        public synchronized void draw(float[] points, float radius, float angle, float left, float top, float right, float bottom) {
        ...
        public void run() {
            ...
            while (running) {
                ...
                if (mQueue.size() > 0 && !isInterrupted()) {
                    ...
                    TiltShiftMode mode = element.tiltShiftMode; // tilt shift mode
