...
public class BlemishPanel extends AbstractContentPanel
    implements ImageViewSpotSingleTap.OnTapListener, OnClickListener, OnItemsScrollListener {
    ...
    public BlemishPanel(IAviaryController context, ToolEntry entry, ToolLoaderFactory.Tools filterType) {
        super(context, entry);
    ...
    public void onCreate(Bitmap bitmap, Bundle options) {
        ...
        minRadiusSize = (float) config.getInteger(R.integer.aviary_spot_gallery_item_min_size) / 100;
        maxRadiusSize = (float) config.getInteger(R.integer.aviary_spot_gallery_item_max_size) / 100;
        ...
        mBackgroundDrawThread =
            new BackgroundDrawThread("draw-thread", Thread.NORM_PRIORITY, mFilter, mThreadHandler, BRUSH_MULTIPLIER);
    ...
    public void onActivate() {
        ...
        if (AviaryOverlay.shouldShow(getContext(), AviaryOverlay.ID_BLEMISH)) {
            mOverlay =
                new BlemishOverlay(getContext().getBaseContext(), R.style.AviaryWidget_Overlay_Blemish, mImageView, mLensButton);
    ...
    public void onClick(View v) {
        ...
        if (id == mLensButton.getId()) {
            setSelectedTool(
                ((ImageViewSpotSingleTap) mImageView).getDrawMode() == TouchMode.DRAW ? TouchMode.IMAGE : TouchMode.DRAW);
        }
    }
    ...
    class GenerateResultTask extends AviaryAsyncTask<Void, Void, MoaActionList> {
        ...
        @Override
        protected void doPostExecute(MoaActionList result) {
