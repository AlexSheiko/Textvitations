...
public abstract class AviaryStoreWrapperAbstract {
    ...
    protected int purchaseRequestCode = -1;
    private Callback callback;

    public AviaryStoreWrapperAbstract(Callback callback, int requestCode) {
        this.callback = callback;
        this.purchaseRequestCode = requestCode;
    }

    public void onAttach(Activity activity, String billingKey) {
        logger.info("onAttach");
        this.context = activity;
        this.tracker = AviaryTracker.getInstance(context);
        this.wrapper = createWrapper(activity, billingKey);
    }

    protected abstract IAPInstance createWrapper(Activity activity, String billingKey);

    public void registerReceivers() {
        logger.info("registerReceivers");
        if (null != context) {
            context.registerReceiver(packPurchasedReceiver,
                                     new IntentFilter(context.getPackageName() + CdsUtils.BROADCAST_PACK_PURCHASED));
            context.registerReceiver(packInstalledReceiver,
                                     new IntentFilter(context.getPackageName() + CdsUtils.BROADCAST_PACK_INSTALLED));
            context.registerReceiver(downloadStatusReceiver,
                                     new IntentFilter(context.getPackageName() + CdsUtils.BROADCAST_DOWNLOAD_STATUS_CHANGED));
            context.registerReceiver(subscriptionPurchasedReceiver,
                                     new IntentFilter(context.getPackageName() + CdsUtils.BROADCAST_SUBSCRIPTION_PURCHASED));
            context.getContentResolver()
                .registerContentObserver(PackageManagerUtils.getCDSProviderContentUri(context, "service/finished"),
                                         false,
    ...
    public void purchase(
        final long packid, final String identifier, final String packType, final String whereFrom, final String price) {
        purchase(packid, identifier, packType, whereFrom, price, -1);
    }

    public void purchase(
        final long packid, final String identifier, final String packType, final String whereFrom, final String price,
        final int clickFromPosition) {
        ...
        IabHelper.OnIabPurchaseFinishedListener purchaseFinishedListener = new IabHelper.OnIabPurchaseFinishedListener() {
            public void onIabPurchaseFinished(IabResult result, Purchase purchase) {
                ...
                tracker.trackEndPurchaseFlow(identifier, whereFrom, result.isSuccess(), clickFromPosition);
            }
        };

        if (null != wrapper && wrapper.isAvailable()) {
            launchPurchaseFlow(identifier, purchaseFinishedListener, null);
    ...
    private void onPurchaseSuccess(
        final Purchase purchase, final long packId, final String identifier, final String packType, final String price) {
    ...
    protected abstract void launchPurchaseFlow(
        String identifier, IabHelper.OnIabPurchaseFinishedListener purchaseFinishedListener, String extraData);
    ...
    void sendReceipt(final Purchase purchase, String price) {
        ...
        try {
            ...
            ReceiptManager.getInstance(context).sendTicket(receipt);
    ...
    public void restore(
        final long packid, final String identifier, final String packType, final String whereFrom, final boolean isRestore,
        final boolean isFree, final boolean isError, final boolean isSubscription) {
        logger.info("restore {%d, %s, %s, %s, restore: %b, free: %b, error: %b, subscription: %s}",
                    packid,
                    identifier,
                    packType,
                    whereFrom,
    ...
    void sendReceipt(final String identifier, boolean free, boolean isRestore, boolean isSubscription) {
        ...
        logger.info("sendReceipt{ identifier: %s, isFree: %b, isRestore: %b, isSubscription: %b }",
                    identifier,
                    free,
                    isRestore,
                    isSubscription);
