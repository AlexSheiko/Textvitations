...
public abstract class AbstractPanel {
    ...
    public interface OnPreviewListener {
         ...
         * @param resetMatrix reset the imageview
         */
        void onPreviewChange(Bitmap result, boolean resetMatrix);

        /**
         * Same as {@link #onPreviewChange(android.graphics.Bitmap, boolean)} but using
         * a drawable instead
         *
         * @param drawable
         * @param resetMatrix
         */
        void onPreviewChange(Drawable drawable, boolean resetMatrix);
    ...
    final Handler mListenerHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case PREVIEW_DRAWABLE_CHANGED:
                    if (mListener != null && isActive()) {
                        mListener.onPreviewChange((Drawable) msg.obj, msg.arg1 == 1);
                    }
                    break;
     ...
     *
     * @param bitmap  the current original bitmap
     * @param options a custom set of initialization options
     */
     ...
     * @param reset  if true, then will ask to reset the imageview to its original matrix
    ...
    protected void onPreviewChanged(Drawable drawable, boolean reset, boolean notify) {
        setIsChanged(drawable != null);
        if (notify && isActive()) {
            Message msg = mListenerHandler.obtainMessage(PREVIEW_DRAWABLE_CHANGED);
            msg.arg1 = reset ? 1 : 0;
            msg.obj = drawable;
            msg.sendToTarget();
        }
    ...
    protected void onComplete(Bitmap bitmap, EditToolResultVO editResult) {
        ...
        if (mApplyListener != null && isActive()) {
            ...
            getContext().getTracker()
                .tagEventAttributes(ToolLoaderFactory.getToolName(mEntryName) + ": saved", mTrackingAttributes);
    ...
    protected void onGenericError(CharSequence message, int yesLabel, OnClickListener yesListener) {
        if (mErrorListener != null && isActive()) {
            mErrorListener.onError(message, yesLabel, yesListener);
        }
    }
    ...
    protected void onGenericError(
        String message, int yesLabel, OnClickListener yesListener, int noLabel, OnClickListener noListener) {
        if (mErrorListener != null && isActive()) {
    ...
    protected void onGenericMessage(
        CharSequence title, CharSequence message, int yesLabel, OnClickListener yesListener, int noLabel,
        OnClickListener noListener) {
        if (null != mErrorListener && isActive()) {
