...
public class StickersOverlay extends AviaryOverlay {
    ...
    public StickersOverlay(final Context context, int styleId, View view, ToolLoaderFactory.Tools toolName, int tutorialId) {
        super(context, ToolLoaderFactory.getToolName(toolName), styleId, tutorialId);
    ...
    public void setAlpha(final float alpha) {
        if (null != mTitleLayout) {
            mTitleLayout.getPaint().setAlpha((int) (alpha * 255));
        }

        if (null != mDetailsLayout) {
            mDetailsLayout.getPaint().setAlpha((int) (alpha * 255));
        }
    ...
    protected void dispatchDraw(final Canvas canvas) {
        ...
        if (USE_CIRCLE) {
            if (Build.VERSION.SDK_INT <= ApiHelper.VersionCodes.HONEYCOMB) {
                Path path = new Path();
                path.addCircle(viewRect.centerX(), viewRect.centerY(), viewRect.width() / CIRCLE_SIZE, Path.Direction.CW);
        ...
        if (USE_CIRCLE) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                canvas.drawCircle(viewRect.centerX(), viewRect.centerY(), viewRect.width() / CIRCLE_SIZE, eraser);
            }
        }

        arrow.setBounds(viewRect.centerX(),
                        (int) (viewRect.top - arrow.getIntrinsicHeight() - (10 * mDensity)),
                        viewRect.centerX() + arrow.getIntrinsicWidth(),
                        (int) (viewRect.top - (10 * mDensity)));
        arrow.draw(canvas);
    ...
    private void calculateTextLayouts() {
        ...
        if (null == mDetailsLayout || null == mTitleLayout || mOrientationChanged) {
            ...
            int textWidth = (int) ((metrics.widthPixels) * (mTextWidthFraction / 100));
            ...
            mDetailsPosition.top =
                (int) (top - arrow.getIntrinsicHeight() - mDetailsLayout.getHeight() - getTextMargins() - (10 * mDensity));
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        int radius = (int) (viewRect.width() / CIRCLE_SIZE);
