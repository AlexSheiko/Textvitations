...
public class AviaryWheel extends View implements OnGestureListener, ScrollableView, VibrationWidget {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int maxWidth = (int) (w - (mEdgeOffset * 4));
    ...
    private void createShader(int w, int h) {
        ...
        final int indicatorWidth = mLinesIndicator.getIntrinsicWidth();
        int left = w2 - indicatorWidth / 2;
        int right = w2 + indicatorWidth / 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null != mShader && mShaderMatrix != null) {
            ...
            int scrollX = getPositionFromValue(mCurrentValue);
    ...
    public void scrollIntoSlots() {
        ...
        if (scrollX > maxX) {
            mScroller.startUsingDistance(scrollX, maxX - scrollX);
            return;
        } else if (scrollX < minX) {
            mScroller.startUsingDistance(scrollX, minX - scrollX);
            return;
        }
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        int currentX = getPositionFromValue(mCurrentValue);
        int newX = (int) (currentX + delta);
        int maxX = getMaxX();
        int minX = getMinX();

        if (!mToLeft) {
            if (currentX + delta > maxX) {

                delta = Math.min(delta, MAX_DELTA);
                delta *= 1.0 - (((mOverscrollX + delta) - maxX) / MAX_DELTA);

                mOverscrollX = currentX + delta;
            ...
            if (currentX + delta < minX) {

                delta = Math.max(delta, -MAX_DELTA);
                delta *= 1 - (((currentX - delta) - minX) / MAX_DELTA);
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (!toleft) {
            ...
            if (currentX > maxX) {
                mScroller.startUsingDistance(currentX, maxX - currentX);
                return true;
            }
        } else {
            ...
            if (currentX < minX) {
                mScroller.startUsingDistance(currentX, minX - currentX);
                return true;
            }
        }

        mScroller.startUsingVelocity(currentX, (int) velocityX / 2);
        return true;
    }
