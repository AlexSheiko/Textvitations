...
public class ImageViewTiltiShiftTouch extends ImageViewTouch {
    ...
    public static final  double CLOUD_INNER_RADIUS_RATIO = 2.5;
    ...
    protected long   mFadeOutDuration       = DEFAULT_ANIMATION_DURATION;
    /** The m current scale. */
    ...
    protected void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        mAnimator = ObjectAnimator.ofFloat(mPointCloud.waveManager, "radius", 0, 100);
     ...
     *
     * @param crossRadius Radius of the crosshair path
     * @param crossEdge   Extra space for the crosshair lines
     */
    public void setCrossHairSize(int crossRadius, int crossEdge) {
        mCrossPath.reset();

        if (mCrossEnabled) {
            mCrossPath.addCircle(0, 0, crossRadius, Direction.CW);

            mCrossPath.moveTo(-crossRadius, 0);
            mCrossPath.lineTo(-crossRadius - crossEdge, 0);

            mCrossPath.moveTo(crossRadius, 0);
            mCrossPath.lineTo(crossRadius + crossEdge, 0);

            mCrossPath.moveTo(0, -crossRadius);
            mCrossPath.lineTo(0, -crossRadius - crossEdge);
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (null == mFirstPointOriginal) {
                    initializeTouch(x - mMinShapeSizePixels / 2,
                                    y - mMinShapeSizePixels / 2,
                                    x + mMinShapeSizePixels / 2,
                                    y + mMinShapeSizePixels / 2);
    ...
    private void initializeTouch(float left, float top, float right, float bottom) {
        ...
        if (null != mPointCloud && mPointCloudEnabled) {
            ...
            float inner = (float) (mCurrentDistance / CLOUD_INNER_RADIUS_RATIO);

            if (mPointCloudEnabled) {
                if (null != mAnimator) {
                    mAnimator.cancel();
                }
                mAnimator.setFloatValues(inner, mPointCloudOuterRadius + 100);
                mAnimator.start();
            }
        }
    }
    ...
    private void updateBitmapRect() {
        ...
        rectChanged = (mBitmapRect == null && rect != null) || (mBitmapRect != null && !mBitmapRect.equals(rect));
        ...
        if (null != rect) {
            ...
            if (null != mBitmapRect) {
                ...
                float oldLeft = mBitmapRect.left;
                float oldTop = mBitmapRect.top;

                float oldWidth = mBitmapRect.width();
                float oldHeight = mBitmapRect.height();

                float diffW = rect.width() / oldWidth;
                float diffH = rect.height() / oldHeight;
                ...
                if (update) {
                    ...
                    mCenterPoint.offset(-oldLeft, -oldTop);
                    mCenterPoint.x *= diffW;
                    mCenterPoint.y *= diffH;
        ...
        if (rectChanged && mPointCloudEnabled) {
            resetWave(mBitmapRect);
        }
    }
    ...
    private void onTouchMove(PointF center, float distance, float angle, boolean firstTime) {
        ...
        if (mTiltShiftDrawMode == TiltShiftDrawMode.Radial) {
            ...
            mShapeRect.set(center.x - radius * BRUSH_MULTIPLIER,
                           center.y - radius * BRUSH_MULTIPLIER,
                           center.x + radius * BRUSH_MULTIPLIER,
                           center.y + radius * BRUSH_MULTIPLIER);
            ...
            mDrawingRect.set(center.x - radius,
                             center.y - mDrawingRectLength / 2,
                             center.x + radius,
                             center.y + mDrawingRectLength / 2);
            mDrawingRect.sort();
        ...
        if (mDrawListener != null) {
            ...
            if (firstTime) {
                mDrawListener.onDrawStart(mappedPoints,
                                          mTiltShiftDrawMode,
                                          (radius / mCurrentScale),
                                          -angle - 90,
                                          mShapeRectInverted.left,
                                          mShapeRectInverted.top,
                                          mShapeRectInverted.right,
                                          mShapeRectInverted.bottom);
            } else {
                mDrawListener.onDrawing(mappedPoints,
                                        (radius / mCurrentScale),
                                        -angle - 90,
                                        mShapeRectInverted.left,
                                        mShapeRectInverted.top,
    ...
    protected void fadeOutShape(long timeout) {
        ...
        ObjectAnimator anim1 = ObjectAnimator.ofInt(this, "paintAlpha", mPaintAlpha, 0);
        ObjectAnimator anim2 = ObjectAnimator.ofInt(this, "backPaintAlpha", mBackPaintAlpha, 0);

        mFadeAnimator.playTogether(anim1, anim2);
        mFadeAnimator.setDuration(mFadeOutDuration);
        mFadeAnimator.setStartDelay(timeout);
        mFadeAnimator.setInterpolator(new ExpoInterpolator(EasingType.Type.OUT));
        mFadeAnimator.start();
    }
    ...
    public void setPaintStrokeColor(int value, int value2) {
        mPaintAlpha = Color.alpha(value);
        mPaintAlphaDefault = mPaintAlpha;
        mBackPaintAlpha = Color.alpha(value2);
    ...
    protected void onDrawModeChanged() {
        ...
        if (null != mBitmapRect) {
            ...
            float size = Math.min(mBitmapRect.width(), mBitmapRect.height()) * DEFAULT_SIZE_RATIO;
    ...
    @SuppressWarnings ("unused")
    public void setBackPaintAlpha(int value) {
        mBackPaintAlpha = value;
        postInvalidate();
    }
    ...
    public interface OnTiltShiftDrawListener {
        ...
        void onDrawStart(
            float[] center, TiltShiftDrawMode drawMode, float radius, float angle, float left, float top, float right,
            float bottom);
