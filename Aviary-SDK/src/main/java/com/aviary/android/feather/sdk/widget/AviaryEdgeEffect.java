...
public class AviaryEdgeEffect {
    ...
    public AviaryEdgeEffect(Context context, int styleid) {
        ...
        mMaxEffectHeight =
            (int) (Math.min(mGlowHeight * MAX_GLOW_HEIGHT * mGlowHeight / mGlowWidth * 0.6f, mGlowHeight * MAX_GLOW_HEIGHT) + 0.5f);
    ...
    public void setColorFilter(int color, PorterDuff.Mode mode) {
        if (null != mEdge) {
            mEdge.setColorFilter(color, mode);
        }
        if (null != mGlow) {
            mGlow.setColorFilter(color, mode);
        }
    }
    ...
    public void onPull(float deltaDistance) {
        ...
        mEdgeAlphaStart = Math.max(PULL_EDGE_BEGIN, Math.min(distance, MAX_ALPHA));
        mEdgeAlpha = mEdgeAlphaStart;
        mEdgeScaleYStart = Math.max(HELD_EDGE_SCALE_Y, Math.min(distance * PULL_DISTANCE_EDGE_FACTOR, 1.f));
        mEdgeScaleY = mEdgeScaleYStart;

        mGlowAlphaStart = Math.min(MAX_ALPHA, mGlowAlpha + (Math.abs(deltaDistance) * PULL_DISTANCE_ALPHA_GLOW_FACTOR));
        ...
        mGlowScaleYStart = Math.min(MAX_GLOW_HEIGHT, Math.max(0, mGlowScaleY + glowChange * PULL_DISTANCE_GLOW_FACTOR));
