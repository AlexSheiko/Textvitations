...
public class StickersPanel extends AbstractContentPanel
    implements OnStatusChangeListener, OnItemClickListener, DragListener, DragSource, DropTargetListener,
    ...
    private View                mOpacityConfirm, mOpacityDismiss;
    ...
    public void onLoadComplete(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (hasOptions() && mFirstTime && !checkFromIap) {
            ...
            mLogger.verbose("options_pack_id: %s, options_content_id: %s", optionsPackId, optionsContentId);
        ...
        if (null != cursor) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (type == TrayColumns.TYPE_PACK_INTERNAL) {
                    ...
                    mLogger.log("%d = %s", packId, identifier);

                    tmpList.add(packId);

                    if (!mInstalledPacks.contains(packId) && !mFirstTime) {
                        mNewPacks.add(packId);
                        mLogger.log("adding %d (%s) to new packs", packId, identifier);
                        mLogger.verbose("iapDialogFeaturedId: %d, pack_id: %d", mLastInstalledPack, packId);

                        if (checkFromIap) {
                            if (iapDialogFeaturedId == packId && mLastInstalledPack == -1) {
                                mLastInstalledPack = packId;
                        ...
                        if (optionsPackId == packId && mPackInfo == null) {
                            mLogger.log("found pack to auto-select: %s", identifier);

                            lastInstalledPackIndex = -1;
                            optionsPackId = -1;

                            mPackInfo = new StickerPackInfo(cursor.getLong(TrayColumns.ID_COLUMN_INDEX),
                                                            cursor.getString(TrayColumns.IDENTIFIER_COLUMN_INDEX));
        ...
        onStickersPackListUpdated(cursor, firstValidIndex, forceUpdate, smoothSelection);
    ...
    private boolean createTutorialOverlayIfNecessaryDelayed(final int firstValidIndex) {
        ...
        if (!isActive() || mStatus.getCurrentStatus() != STATUS_PACKS) {
            return false;
        }
            ...
            if (null != view) {
                ...
                if (null != tag && tag instanceof ViewHolder) {
                    ...
                    if (holder.type == StickerPacksAdapter.TYPE_EXTERNAL) {
                        ...
                        free = viewHolderExternal.free == 1;
                        if (free) {
                            validIndex = i;
                            validView = viewHolderExternal.image;
                        }
                    }
                }
            }
    ...
    public void onCreate(Bitmap bitmap, Bundle options) {
        ...
        mSeekBar = (SeekBar) getOptionView().findViewById(R.id.aviary_seekbar);
        ...
        mSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(
                ...
                if (hv != null) {
                    double alpha = (double) progress / 100 * 255;
                    hv.setOpacity((int) alpha);
                }
            }
            ...
            public void onStopTrackingTouch(final SeekBar seekBar) {
                if (!ApiHelper.AT_LEAST_14) {
                    final DrawableHighlightView hv = ((ImageViewDrawableOverlay) mImageView).getHighlightViewAt(0);
                    hv.invalidateDrawable(hv.getContent().getCurrent());
                }
            }
        });

        mOpacityDismiss = getOptionView().findViewById(R.id.aviary_button_cancel);
        mOpacityDismiss.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
                mLogger.log("sticker opacity dismiss clicked");
                final DrawableHighlightView hv = ((ImageViewDrawableOverlay) mImageView).getHighlightViewAt(0);
                if (hv != null) {
                    hv.setOpacity(hv.getConfirmedOpacity());
                    mSeekBar.setProgress((int) ((double) hv.getConfirmedOpacity() / 255 * 100));
                }
                mStatus.setStatus(mStatus.getPreviousStatus());
            }
        });

        mOpacityConfirm = getOptionView().findViewById(R.id.aviary_button_confirm);
        mOpacityConfirm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
                mLogger.log("sticker opacity confirm clicked");
                mStatus.setStatus(mStatus.getPreviousStatus());
                final DrawableHighlightView hv = ((ImageViewDrawableOverlay) mImageView).getHighlightViewAt(0);
                hv.setConfirmedOpacity(hv.getOpacity());
            }
        });
    ...
    public boolean onBackPressed() {
        ...
        // we're in opacity changing status
        if (mStatus.getCurrentStatus() == STATUS_OPACITY) {
            final DrawableHighlightView hv = ((ImageViewDrawableOverlay) mImageView).getHighlightViewAt(0);
            if (hv != null) {
                hv.setOpacity(hv.getConfirmedOpacity());
                mSeekBar.setProgress((int) ((double) hv.getConfirmedOpacity() / 255 * 100));
    ...
    public void onStatusChanged(int oldStatus, int newStatus) {
        ...
        if (oldStatus == STATUS_OPACITY || newStatus == STATUS_OPACITY) {
            inAnimation = R.anim.aviary_bottobar_in_animation;
            outAnimation = R.anim.aviary_bottobar_out_animation;
        } else {
            inAnimation = R.anim.abc_fade_in;
            outAnimation = R.anim.abc_fade_out;
        }

        mViewFlipper.setInAnimation(getContext().getBaseContext(), inAnimation);
        mViewFlipper.setOutAnimation(getContext().getBaseContext(), outAnimation);
        ...
        if (mStatus.getCurrentStatus() == STATUS_PACKS) {
            ...
            if (null != holder) {
                ...
                if (holder.type == StickerPacksAdapter.TYPE_LEFT_GETMORE || holder.type == StickerPacksAdapter.TYPE_RIGHT_GETMORE) {
                    ...
                    attrs.put("from", ToolLoaderFactory.Tools.STICKERS.name().toLowerCase(Locale.US));
                    ...
                    getContext().getTracker().tagEventAttributes("shop_list: opened", attrs);

                    IAPUpdater iapData = new IAPUpdater.Builder().setPackType(PackType.STICKER).setFeaturedPackId(-1).build();
                    displayIAPDialog(iapData);
                    ...
                    getContext().getTracker().tagEventAttributes("shop_details: opened", attrs);

                    Bundle extras = new Bundle();
                    extras.putInt(IAPDialogDetail.EXTRA_CLICK_FROM_POSITION, position);

                    IAPUpdater iapData = new IAPUpdater.Builder().setPackType(PackType.STICKER)
                        .setPackId(holder.id)
    ...
    private boolean startDrag(
        it.sephiroth.android.library.widget.AdapterView<?> parent, View view, int position, long id, boolean animate) {

        if (parent == null || view == null || parent.getAdapter() == null || mStatus.getCurrentStatus() != STATUS_STICKERS) {
            return false;
        }
        ...
        if (null != view) {
            ...
            if (null != image) {
                if (null == parent.getAdapter() || !(parent.getAdapter() instanceof StickersAdapter)) {
                    return false;
                }
                ...
                getContext().getTracker().tagEvent("stickers: drag_began", "item", identifier);
                ...
                if (null != drawable && drawable instanceof BitmapDrawable) {
                    ...
                    bitmap = Bitmap.createScaledBitmap(src,
                                                       (int) (src.getWidth() * DRAG_SCALE_FACTOR),
                                                       (int) (src.getHeight() * DRAG_SCALE_FACTOR),
                                                       true);
                    scaleFactor = DRAG_SCALE_FACTOR;
                }
                ...
                try {
                    ...
                    return getDragController().startDrag(image,
                                                         bitmap,
                                                         StickersPanel.this,
                                                         new StickerDragInfo(contentPath, identifier, scaleFactor),
                                                         DragControllerService.DRAG_ACTION_MOVE,
                                                         loc[0],
                                                         loc[1]);
                ...
                return getDragController().startDrag(image,
                                                     StickersPanel.this,
                                                     new StickerDragInfo(contentPath, identifier, 1),
                                                     DragControllerService.DRAG_ACTION_MOVE,
                                                     animate);
            }
        }
    ...
    public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo) {
        ...
        mLogger.info("onDrop. source: %s, x: %d, y: %d, xOffset: %d, yOffset: %d", source, x, y, xOffset, yOffset);
        ...
        if (dragInfo != null && dragInfo instanceof StickerDragInfo) {
            ...
            float scaleFactorX = dragView.getScaleFactorX();
            float scaleFactorY = dragView.getScaleFactorY();

            float w = dragView.getWidth();
            float h = dragView.getHeight();

            mLogger.log("w: %f, h: %f", w, h);

            int width = (int) (w / info.scaleFactor);
            int height = (int) (h / info.scaleFactor);

            int targetX = x - xOffset;
            int targetY = y - yOffset;

            if (info.scaleFactor != 1) {
                targetX -= (w / info.scaleFactor) / 2;
                targetY -= (h / info.scaleFactor) / 2;
            }
        ...
        if (null == mCursorLoaderPacks) {
            ...
            context.getContentResolver()
                .registerContentObserver(PackageManagerUtils.getCDSProviderContentUri(context,
                                                                                      "packTray/" + AviaryCds.PACKTYPE_STICKER),
                                         false,
                                         mContentObserver);
        }
    ...
    private StickerPacksAdapter createPacksAdapter(Context context, Cursor cursor) {
        return new StickerPacksAdapter(context,
                                       R.layout.aviary_sticker_item,
    ...
    protected void loadStickers() {
        ...
        Cursor cursor = context.getContentResolver()
            .query(PackageManagerUtils.getCDSProviderContentUri(context, "pack/" + mPackInfo.packId + "/item/list"), new String[]{
                PacksItemsColumns._ID + " as _id", PacksItemsColumns._ID, PacksItemsColumns.PACK_ID, PacksItemsColumns.IDENTIFIER,
        ...
        mListStickers.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(
                it.sephiroth.android.library.widget.AdapterView<?> parent, View view, int position, long id) {
                StickersAdapter adapter = ((StickersAdapter) parent.getAdapter());
        ...
        if (hasOption(Constants.QuickLaunch.CONTENT_ITEM_ID)) {
            Bundle options = getOptions();
            long itemId = options.getLong(Constants.QuickLaunch.CONTENT_ITEM_ID, -1);
            for (int i = 0; i < mAdapterStickers.getCount(); i++) {
                if (itemId == mAdapterStickers.getItemId(i)) {
                    mListStickers.performItemClick(null, i, itemId);
                    break;
                }
            }
    ...
    private void addSticker(String contentPath, String identifier, RectF position) {
        ...
        if (file.exists()) {
            StickerDrawable drawable = new StickerDrawable(getContext().getBaseContext().getResources(),
    ...
    private void addSticker(FeatherDrawable drawable, RectF positionRect) {
        final DrawableHighlightView hv =
            new DrawableHighlightView(mImageView, ((ImageViewDrawableOverlay) mImageView).getOverlayStyleId(), drawable);
        ...
        hv.setOnContentFlipListener(new DrawableHighlightView.OnContentFlipListener() {
            @Override
            public void onContentFlip(final DrawableHighlightView content) {
                getContext().getTracker().tagEvent("stickers: item_flipped");
            }
        });
        hv.setOnChangeOpacityListener(new DrawableHighlightView.OnOpacityChangeListener() {
            @Override
            public void onChangeOpacity() {
                // switch to opacity mode
                mStatus.setStatus(STATUS_OPACITY);
            }
        ...
        if (cropSize > screenSize) {
            ...
            if (positionRect == null) {
                ...
                positionRect =
                    new RectF(w / 2 - cropWidth / 2, h / 2 - cropHeight / 2, w / 2 + cropWidth / 2, h / 2 + cropHeight / 2);
            }
        ...
        if (mStickerFlipToolTip++ == 0) {

            int actionbarsize = ((FeatherContext) getContext().getBaseContext()).getActionBarSize();
            ...
            if (null != tooltipManager) {
                tooltipManager.create(0)
                    ...
                    .anchor(new Point(x + cropWidth / 2, y - 5), TooltipManager.Gravity.TOP)
                    .text(getContext().getBaseContext().getResources(), R.string.feather_sticker_flip_tooltip)
                    .closePolicy(TooltipManager.ClosePolicy.TouchOutside, 3000)
                    .activateDelay(1000)
                    .maxWidth(mImageView.getWidth() / 2)
                    .showDelay(150)
            ...
            if (null != mPreferenceService) {
                mPreferenceService.containsSingleTimeKey(((Object) StickersPanel.this).getClass(), "sticker.flip.tooltip", true);
            }

            hv.playHorizontalFlipDemo();
        }
    ...
    private void onApplyCurrent() {
        ...
        if (hv != null) {
            ...
            mImageView.getImageMatrix().invert(matrix);
            ...
            if (mCurrentFilter != null) {
                ...
                mCurrentFilter.setFlipH(hv.getFlipHorizontal());
                mCurrentFilter.setAlpha((int) ((double) mSeekBar.getProgress() / 100 * 255));
    ...
    private void onClearCurrent(DrawableHighlightView hv, boolean removed) {
        ...
        hv.setOnContentFlipListener(null);
    ...
    private void onStickersPackListUpdated(Cursor cursor, int firstIndex, boolean forceSelection, final boolean smoothSelection) {
                ...
                if (holder.id != id) {
                    mPicassoLib.load(icon)
                        ...
                        .transform(new PackIconCallable.Builder().withResources(getContext().getBaseContext()
                                                                                    .getResources()).withPackType(PackType.STICKER)
                                                /*.withAlpha( 153 )*/.withPath(icon).build())
                        .noFade()
    ...
    private void shakePack(ViewHolder holder) {
        if (null == holder || null == holder.image) {
            return;
        }
        ...
        scaleAnimation.playTogether(
            ObjectAnimator.ofInt(holder.image, "alpha", 0, 255),
            ObjectAnimator.ofFloat(holder.image, "scaleX", SHAKE_PACK_SCALE_START, 1f),
            ObjectAnimator.ofFloat(holder.image, "scaleY", SHAKE_PACK_SCALE_START, 1f)
        );

        Animator shakeAnimation = ObjectAnimator.ofFloat(holder.image, "rotation", 0f, SHAKE_PACK_ROTATION_END);
    ...
    class StickersAdapter extends CursorAdapter {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            mPicassoLib.load(iconPath).skipMemoryCache().resize(mStickerThumbSize, mStickerThumbSize, true).noFade().into(image);
    ...
    static class StickerDragInfo {
        ...
        StickerDragInfo(String contentPath, String identifier, float scaleFactor) {
